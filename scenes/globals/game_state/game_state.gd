...
func start_quest(quest: Quest) -> void:
	...
	_state.set_value(QUEST_SECTION, QUEST_CURRENTSCENE_KEY, quest.first_scene)
	current_spawn_point = ^""
...
func _restore() -> void:
	...
	var current_spawn_point: NodePath = _state.get_value(QUEST_SECTION, QUEST_SPAWNPOINT_KEY, ^"")
	if scene_path:
